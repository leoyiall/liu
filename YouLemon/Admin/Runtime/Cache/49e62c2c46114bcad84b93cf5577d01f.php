				...
				<!-- 列表开始 -->
					<div id="gl_list">
						<table cellpadding="0" cellspacing="0">
							<tr>
						<td colspan="7" class="gl_tou">供应商列表</td>
							</tr>
							<tr>
								<th>ID</th>
								<th>用户账号</th>
							...
							<?php if(is_array($all)): $i = 0; $__LIST__ = $all;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><tr>
								<td><?php echo ($vv["gong_id"]); ?></td>
								<td><?php echo ($vv["username"]); ?></td>
								<td><?php echo ($vv["gongName"]); ?></td>
								<td><?php echo ($vv["gongPhone"]); ?></td>
								<td><img src="__PUBLIC__/Images/tou/<?php echo ($vv["gongImg"]); ?>" width="30" height="30"></td>
								<td>
								<ul>
								<!-- 审批开通店铺 -->
								<?php if(($vv["is_kai"]) == "1"): ?><a href="__APP__/Houshop/gongshen?name='<?php echo ($vv["username"]); ?>'&id=2" ><li class="btn_se">通过审核</li></a>
									<a href="__APP__/Houshop/gongshen?name='<?php echo ($vv["username"]); ?>'&id=0" ><li class="btn_se">不通过</li></a><?php endif; ?>
								<!-- 开通后封店 -->
								<?php if(($vv["is_kai"]) == "2"): ?><a href="__APP__/Houshop/gongshen?name='<?php echo ($vv["username"]); ?>'&id=3" onclick="return confirm('确认封店吗?');"><li class="del">封店</li></a><?php endif; ?>
								<!-- 封店后重新开启 -->
								<?php if(($vv["is_kai"]) == "3"): ?><a href="__APP__/Houshop/gongshen?name='<?php echo ($vv["username"]); ?>'&id=2" onclick="return confirm('确认重新开启店铺?');"><li class="del btn_se" >开启</li></a><?php endif; ?>								
								</ul>
								</td>
