...
<div class="mainContent">
  <div class="ulist">
      ...
      <?php if(!$ilist['blogTitle']): echo ($list['idnumber']); ?>的博客</a>
      <?php else: ?>
...
<div class="mainContent2">
      ...
      <div class="userinfo"> 
        ...
        <p class="btns"><a href="/" target="_blank" >私信</a><a href="/" target="_blank">关注</a></p>   
      </div>
   ...
   <form action="<?php echo U('doModifyBlog');?>" method="post">
      <table>
      <tr>
        <th>博文标题:</th>
        <td>
          <input type="text" name="title" autofocus class="input_style" value="<?php echo ($bli['title']); ?>">
          <input type="hidden" name="user_id" value="<?php echo ($id); ?>" class="input_style">
        	<input type="hidden" name="lid" value="<?php echo ($bli['lid']); ?>" class="input_style">
        </td>
      </tr>
      <tr>
        <th>文章分类:</th>
        <td>
          <select name="cate_id">
          <option value="<?php echo ($bli['cate_id']); ?>"><?php echo ($bli['category']); ?></option>
       <?php if(is_array($cate)): $i = 0; $__LIST__ = $cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vv["caid"]); ?>"><?php echo ($vv["category"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        ...
        <div class="content">
          <textarea id="editor" name="content" placeholder="这里输入内容"><?php echo ($bli['content']); ?></textarea>
