...
<tr>
  <th colspan="7">
<form action="<?php echo U('courseList');?>" method="post">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
    <td><a href="__ROOT__/index.php/Programme/teachProgramme.shtml?id=<?php echo ($vo["pid"]); ?>&kid=<?php echo ($vo["id"]); ?>" target="_blank"><img src="__PUBLIC__/Uploads/learn/<?php echo ($vo["logo"]); ?>" width="30" height="30" alt="<?php echo ($vo["course"]); ?>"></a></td>
  	<td><a href="__ROOT__/index.php/Programme/teachProgramme.shtml?id=<?php echo ($vo["pid"]); ?>&kid=<?php echo ($vo["id"]); ?>" target="_blank"><?php echo ($vo["course"]); ?></a></td>
    ...
    <td><?php echo date('Y-m-d H:i:s',$vo['sendTime']); ?></td>
  	<td>
      <a href="<?php echo U('modifyCourse');?>?id=<?php echo ($vo["id"]); ?>" delId="<?php echo ($vo["qid"]); ?>" class="admin-modify" >修改</a>
      <a href="javascript:void(0);" delId="<?php echo ($vo["id"]); ?>" bs='del' class="admin-modify admin-del" >删除</a>
    <?php if($vo["isOver"] == 0): ?><a href="javascript:void(0);" delId="<?php echo ($vo["id"]); ?>" bs='ov' class="admin-modify admin-del" >完课</a>
    <?php else: ?>
      <a href="javascript:void(0);" delId="<?php echo ($vo["id"]); ?>" bs='nov' class="admin-modify admin-del" >取消结课</a><?php endif; ?>
  	</td>
...
$(".admin-del").on("click",function(){
    if(confirm("是否进行该操作?")){
      ...
      var bs = del.attr("bs");
       $.ajax({
        url : "<?php echo U('toDoIt');?>?id="+aid+"&bs="+bs,
        type:'get',
        datatype:'json',
        success:function(e){
          console.log(e);
