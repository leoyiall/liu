...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
   ...
   <?php if($vo["check"] == 1): ?><span style="color:green">已审</span>
    <?php else: ?>
  	...
  	<td>
      <a href="<?php echo U('doExamine');?>/id/<?php echo ($vo["id"]); ?>/check/1" onclick="confirm(是否要修改该文章?)" class="admin-modify" style="background:green;color:#fff;">通过审核</a>
  		<a href="<?php echo U('doExamine');?>/id/<?php echo ($vo["id"]); ?>/check/3" onclick="confirm(是否要修改该文章?)" class="admin-modify" style="background:red;color:#fff;">不通过审核</a>
