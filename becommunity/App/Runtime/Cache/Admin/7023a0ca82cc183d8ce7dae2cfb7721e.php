...
<div class="alert alert-info" role="alert"><b>注意:</b>点击文字即可进行修改，再点下表单外即可修改章节。</div>
...
<table class="table table-hover">
  <tr>
   ...
   <tr>
   <td></td>
    <td><input type="text" name="state" class="form-control" placeholder="章节名称"  autofocus="autofocus"></td>
    <td>
      <a href="javascript:void(0);" class="admin-modify" style="line-height:35px;"  id="addState">添加</a>
    </td>
  </tr>
<?php $__FOR_START_12360__=1;$__FOR_END_12360__=count($list);for($i=$__FOR_START_12360__;$i < $__FOR_END_12360__;$i+=1){ } ?>
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
    <td><?php echo ($i); ?></td>
    <td class="caname" objId="<?php echo ($vo["id"]); ?>"><?php echo ($vo['zname']); ?></td>
    <td><a href="<?php echo U('delCategory');?>?id=<?php echo ($vo["id"]); ?>" class='admin-modify' objId='<?php echo ($vo["id"]); ?>' onclick="confirm('确定删除该分类吗?')">删除</a></td>
...
$(document).ready(function(){
  ...
  $("#addState").click(function(){
    ...
    $.ajax({
      url : "<?php echo U('programme/doAddSort');?>",
      type:'post',
      datatype:'json',
      data:{
        'state':message
