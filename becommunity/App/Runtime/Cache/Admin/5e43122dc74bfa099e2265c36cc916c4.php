...
<form action="<?php echo U('index');?>" method="post">
    ...
    <input type="text" name="so" class="form-control" placeholder="对源码标题进行搜索..." style="width:200px;float:right;">
  </form>  
  </th>
</tr>
  <tr>
...
<!--     <td>
    ...
    <td><a href="__ROOT__/index.php/Code/downpage?id=<?php echo ($vo["id"]); ?>" target="_blank">《<?php echo ($vo["title"]); ?>》</a></td>
    <td><?php echo ($vo["fl"]); ?></td>
    
    <td><?php echo date('Y-m-d H:i:s',$vo['sendTime']); ?></td>
  	<td>
     <a href="<?php echo ($vo["link"]); ?>" target="_blank">下载</a>
    </td>
  	<td>
      <a href="<?php echo U('modifyCode');?>?id=<?php echo ($vo["id"]); ?>"  class="admin-modify admin-del" >修改</a>
