...
<span class="hot-color">提示：</span>需要详细的写明软件的来源和下载地址，保证下载路径正确。<span class="hot-color">注意区分软件来源。</span><br>如果使用过程中遇到问题可以查阅帮助手册。
</div>
</div>
  <div class="row  admin-box">
  <div class="col-md-12">
    <h4 class="text-center">发布源码</h4>
    <form action="<?php echo U('doModifyCode');?>" method="post" class="form-horizontal" enctype="multipart/form-data" id="subForm">
      <div class="form-group">
        <label for="inputEmail3" class="col-sm-2 control-label">源码名称：</label>
        <div class="col-sm-6">
          <input type="text" class="form-control" id="inputEmail3" name="title" placeholder="源码名称" value="<?php echo ($c['title']); ?>" autofocus="autofocus">
      ...
      <option value='<?php echo ($c["sort"]); ?>'><?php echo ($c["fl"]); ?></option>
     <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vv["cid"]); ?>'><?php echo ($vv["sort"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-5">
      <input type="text" class="form-control" id="inputEmail3" placeholder="源码关键字" name="keywords" value="<?php echo ($c['keywords']); ?>">
        </div>
        ...
        <label for="inputPassword3" class="col-sm-2 control-label">下载路径</label>
            ...
            <div id="outit" style="position:relative;">
                <input type="text" class="form-control" id="inputEmail3" style="margin:5px auto;" placeholder="下载链接" name="link" value="<?php echo ($c['link']); ?>">
                <input type="text" class="form-control" id="inputEmail3" style="margin:5px auto;" placeholder="文件大小" name="softbig" value="<?php echo ($c['big']); ?>">
                 <span style="float:right;position:absolute;right:-160px;top:45px;color:red;">*勿忘输入源码大小单位</span>
                <input type="text" class="form-control" id="inputEmail3" style="margin:5px auto;" placeholder="演示链接" name="look" value="<?php echo ($c['look']); ?>">
              </div>
      ...
      <div class="form-group">
        ...
        <div class="col-sm-6">
      <textarea class="form-control" rows="3"  placeholder="源码介绍" name="introduce" value="<?php echo ($c['introduce']); ?>"></textarea>
      </div>
    </div>
      <div class="form-group">
        <label for="inputPassword3" class="col-sm-2 control-label">系统使用</label>
        <div class="col-sm-6">
          <script id="editor" type="text/plain" style="width:800px;height:200px;"  name="content">
          <?php echo ($c['content']); ?></script>
      </div>
    </div>
     <div class="form-group">
