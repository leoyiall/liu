...
<ol class="breadcrumb">
  ...
  <div class="col-md-12">
    <h4 class="text-center">添加课程章节</h4>
    <form action="<?php echo U('doAddChapter');?>" method="post" class="form-horizontal" enctype="multipart/form-data" id="subForm" enctype="multipart/form-data">
      <div class="form-group">
        <label for="inputEmail3" class="col-sm-2 control-label">章节名称：</label>
        <div class="col-sm-6">
          <input type="text" class="form-control" id="inputEmail3" name="title" placeholder="章节名称" autofocus="autofocus">
        </div>
      </div>
     ...
     <?php if(is_array($lz)): $i = 0; $__LIST__ = $lz;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vv["id"]); ?>'><?php echo ($vv["zname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
      ...
      <div class="form-group">
        <div class="col-sm-offset-2 col-sm-10">
          <button type="button" class="btn btn-default"style="margin-left:200px;" id="sub">添加章节名称</button>
        </div>
      </div>
    </form>
  </div>
