...
<span class="hot-color">提示：</span>软件标签需要填写，只有这样才能查询到对应的软件安装和软件教程。<span class="hot-color">注意区分软件来源。</span><br>如果使用过程中遇到问题可以查阅帮助手册。
	...
	<div class="col-md-12">
		...
		<form action="<?php echo U('modifySoft');?>" method="post" class="form-horizontal" enctype="multipart/form-data">
		<input type="hidden" name="id" value="<?php echo ($list[0][id]); ?>">
		  <div class="form-group">
		    <label for="inputEmail3" class="col-sm-2 control-label">软件名称：</label>
		    <div class="col-sm-6">
		      <input type="text" class="form-control" id="inputEmail3" name="title" placeholder="软件名称" autofocus="autofocus" value="<?php echo ($list[0][title]); ?>">
		    </div>
		  </div>
		  <div class="form-group">
		...
		<option value='<?php echo ($list[0][sort]); ?>'><?php echo ($list[0][sort]); ?></option>
		   <option value='开发工具'>开发工具</option>
		    ...
		    <div class="col-sm-6">
				<select class="form-control" name="language">
				  <option value='<?php echo ($list[0]['language']); ?>'><?php echo ($list[0]['language']); ?></option>
		  ...
		  <div class="form-group">
		    ...
		    <div class="col-sm-6">
				...
				<?php if($list[0]['give'] == '免费'): ?><option value='免费'>免费</option>
				  <option value='收费'>收费</option>
				  <?php else: ?>
				  <option value='免费'>免费</option>
				  <option value='收费'>收费</option><?php endif; ?>
	...
	<?php if(is_array($slist)): $i = 0; $__LIST__ = $slist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["id"] == $list[0]['sid']): ?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["state"]); ?></option>
					<?php else: ?>
			  <option value='<?php echo ($vo["id"]); ?>'><?php echo ($vo["state"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
				</select>
		  </div>
		  </div>
		    ...
		    <label for="inputPassword3" class="col-sm-2 control-label">来源</label>
		        <div class="col-sm-10">
		    		<label >
		    		   <input type="radio" name="soft_id" value="0"  style="width:20px;height:20px;" id="onit" checked="checked" >
		    		   该软件存于百度云，提供下载
		    		   <input type="radio" name="soft_id" value="1" style="width:20px;height:20px;" id="onit2"> 官方下载
		    		<div id="outit" style="position:relative;">
    		   		  <input type="text" class="form-control" id="inputEmail3" style="margin:5px auto;" placeholder="下载链接" name="link" value="<?php echo ($list[0][link]); ?>">
    		   		  <input type="text" class="form-control" id="inputEmail3" style="margin:5px auto;" placeholder="文件大小" name="softbig" value="<?php echo ($list[0][softbig]); ?>">
		  ...
		  <div class="form-group">
		    ...
		    <div class="col-sm-6">
			<textarea class="form-control" rows="3"  placeholder="软件介绍" name="introduce"><?php echo ($list[0][introduce]); ?></textarea>
		  </div>
		</div>
		  <div class="form-group">
		    <label for="inputPassword3" class="col-sm-2 control-label">软件关键字</label>
		    <div class="col-sm-5">
		<input type="text" class="form-control" id="inputEmail3" placeholder="软件关键字" name="keywords" value="<?php echo ($list[0][keywords]); ?>">
		  </div>
		    ...
		    <div class="col-sm-5">
		<input type="text" class="form-control" id="inputEmail3" placeholder="软件公司" name="from" value="<?php echo ($list[0][from]); ?>">
		  </div>
		  </div>
		 ...
		 <div class="form-group">
		  ...
		  <div class="form-group">
		    <div class="col-sm-offset-2 col-sm-10">
		      <button type="submit" class="btn btn-default"style="margin-left:200px;">发布文章</button>
