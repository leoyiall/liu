...
<tr>
  <th colspan="4">
<form action="<?php echo U('chapterlist');?>" method="post">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
  	...
  	<td><?php echo ($vo["kname"]); ?> <?php echo ($vo["zname"]); ?></td>
    <td><?php echo date('Y-m-d H:i:s',$vo['sendTime']); ?></td>
  	<td>
    <a href="<?php echo U('modifyChapter');?>?id=<?php echo ($vo["id"]); ?>" class="admin-modify" >修改</a>
...
$(".admin-del").on("click",function(){
    if(confirm("是否删除该问题?")){
      ...
      var aid = del.attr("delId");
       $.ajax({
        url : "<?php echo U('deleteChapter');?>?id="+aid,
        type:'get',
        datatype:'json',
        success:function(e){
          console.log(e);
