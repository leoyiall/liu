...
<!-- 导航条 -->
<ol class="breadcrumb">
  <li><a href="<?php echo U('Question/index');?>">首页</a></li>
...
<?php $__FOR_START_427__=1;$__FOR_END_427__=count($list);for($i=$__FOR_START_427__;$i < $__FOR_END_427__;$i+=1){ if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
    <td><?php echo ($i); ?></td>
    <td class="caname" objId="<?php echo ($vo["tid"]); ?>"><?php echo ($vo["typename"]); ?></td>
    <td><a href="<?php echo U('delCategory');?>?id=<?php echo ($vo["tid"]); ?>" class='admin-modify' objId='<?php echo ($vo["tid"]); ?>' onclick="confirm('确定删除该分类吗?')">删除</a></td>
...
$(document).ready(function(){
  ...
  $("#addState").click(function(){
    ...
    $.ajax({
      url : "<?php echo U('question/doAddSort');?>",
      type:'post',
      datatype:'json',
      data:{
        'state':message
