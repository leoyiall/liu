...
<tr>
  <th colspan="6">
<form action="<?php echo U('messageManage');?>" method="post">
    <input type="submit" value="搜索" class="btn btn-default" style="float:right;margin-left:5px;">
    <input type="text" name="so" class="form-control" placeholder="对留言进行搜索..." style="width:200px;float:right;">
  </form>  
  </th>
</tr>
  <tr>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
  	<td><a href="__ROOT__/index.php/Person/index?id=<?php echo ($vo["me_id"]); ?>" target="_blank"><img src="__PUBLIC__/Uploads/<?php echo ($vo["arcimg"]); ?>" width="30" height="30" alt="<?php echo ($vo["nicheng"]); ?>"></a></td>
  	<td><a href="__ROOT__/index.php/Person/index?id=<?php echo ($vo["me_id"]); ?>" target="_blank"><?php echo ($vo["nicheng"]); ?></a></td>
    <td><?php echo ($vo["message"]); ?></td>
    
  	<td><?php echo date('Y-m-d H:i:s',$vo['sendTime']); ?></td>
  	<td>
  		<a href="javascript:void(0);" delId="<?php echo ($vo["mid"]); ?>" class="admin-modify admin-del" >删除</a>
  	</td>
...
$(".admin-del").on("click",function(){
    if(confirm("是否删除该文章?")){
      ...
      var aid = del.attr("delId");
       $.ajax({
        url : "<?php echo U('deleteMessage');?>?id="+aid,
        type:'get',
        datatype:'json',
        success:function(e){
          console.log(e);
