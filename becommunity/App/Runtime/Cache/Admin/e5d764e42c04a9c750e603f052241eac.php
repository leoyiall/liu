...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
    ...
    <td><?php echo ($vo["comment"]); ?></td>
  	...
  	<td>
    <?php if($vo['pass'] == 0): ?><a href="javascript:void(0);" objId="<?php echo ($vo["cid"]); ?>" class="admin-modify admin-pass" >审核</a><?php endif; ?>
  		<a href="javascript:void(0);" delId="<?php echo ($vo["cid"]); ?>" class="admin-modify admin-del" >删除</a>
  	</td>
...
$(".admin-pass").on("click",function(){
    if(confirm("该文章是否通过审核?")){
      ...
      var aid = del.attr("objId");
       $.ajax({
        url : "<?php echo U('passComment');?>?id="+aid,
        type:'get',
        datatype:'json',
        success:function(e){
          console.log(e);
