...
class QuestionModel extends Model{
	...
	public function getQuestionList($idnumber,$num){
		...
		switch ($num) {
			case '1':
				$count = $q->where("state='0'")->count();
				break;
			case '2':
				$count = $q->where("state='0' and money>0")->count();
				break;
			case '3':
				$count = $q->where("state !='0'")->count();
				break;
			case '4':
				$count = $q->where("state='0' and sight='0'")->count();
				break;	
			default:
				$count = $q->count();
				break;
		...
		switch($num){
			...
			case "1":
			$bool['list'] = $q->where("state='0'")->order("sendTime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
			break;
			case "2":
			$bool['list'] = $q->where("state='0' and money>0")->order("sendTime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
			break;
			case "3":
			$bool['list'] = $q->where("state !='0'")->order("sendTime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
			break;
			case "4":
			$bool['list'] = $q->where("state='0' and sight='0'")->order("sendTime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
			break;
			default:
	...
	// 获取个人中心对应的问题
	public function getQuestionIndex($idnumber,$num){
		$q = M('question_list');
		import('ORG.Util.Page');// 导入分页类
		switch ($num) {
			case '1':
				$count = $q->where("state='0' and user_id='$idnumber'")->count();
				break;
			case '2':
				$count = $q->where("state='0' and money>0 and user_id='$idnumber'")->count();
				break;
			case '3':
				$count = $q->where("state !='0' and user_id='$idnumber'")->count();
				break;
			case '4':
				$count = $q->where("state='0' and sight='0' and user_id='$idnumber'")->count();
				break;	
			default:
		...
		switch($num){
			// 1为问答，2为悬赏问题,3为已解决，4为未回答
			case "1":
			$bool['list'] = $q->where("state='0' and user_id='$idnumber'")->order("sendTime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
			break;
			case "2":
			$bool['list'] = $q->where("state='0' and money>0 and user_id='$idnumber'")->order("sendTime,money desc")->limit($Page->firstRow.','.$Page->listRows)->select();
			break;
			case "3":
			$bool['list'] = $q->where("state !='0' and user_id='$idnumber'")->order("sendTime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
			break;
			case "4":
			$bool['list'] = $q->where("state='0' and sight='0' and user_id='$idnumber'")->order("sendTime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
			break;
			default:
	...
	public function getList($tj,$limit){
		$ql = M("userintro");
		switch($tj){
			case 'jf'://积分
			$bool = $ql->order("jifen desc")->limit($limit)->select();
			break;
			case 'money'://金币排行
			$bool = $ql->order("bmoney desc")->limit($limit)->select();
			break;
			case 'no'://未解决
			$ll = M('question_list');
			$bool = $ll->where("state='0'")->order("sendTime desc")->limit($limit)->select();
			break;
			case 'my'://没有人回答的
			$ll = M('question_list');
			$bool = $ll->where("state='0'")->order("sendTime desc")->limit($limit)->select();
	...
	public function getCommentList($id){
		$q = M('ques_comment');
		$list = $q->where("ques_id='$id'")->order("'order' desc")->select();
		return $list;
	}
	// 个人中心获取未回答的问题10条
