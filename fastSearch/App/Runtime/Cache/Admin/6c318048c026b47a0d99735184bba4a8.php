 ...
 <body>
  ...
  <nav class="navbar navbar-default" style="position:fixed;width:100%;overflow:hidden;*zoom:1;z-index:10;">
    ...
    <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
      ...
      <td>
        <a href="__ROOT__/index.php/Index/article/id/<?php echo ($vo["id"]); ?>" target="_blank"><?php echo ($vo["title"]); ?></a>
      </td>

      <td>
      <?php if(is_array($cs)): foreach($cs as $key=>$v): if($v['id'] == $vo['cid']): echo ($v["cName"]); endif; endforeach; endif; ?>
      </td>
      <td>
      <?php if(is_array($ss)): foreach($ss as $key=>$s): if($s['id'] == $vo['sid']): echo ($s["state"]); endif; endforeach; endif; ?>
      </td>
      <td>
        <a href="javascript:void(0)" data-link="<?php echo U('modifyArticle');?>?id=<?php echo ($vo["id"]); ?>" class="btn btn-default" onclick="treat(this,'是否修改栏目?')">修改</a>
        <a href="javascript:void(0)" data-link="<?php echo U('delArticle');?>?id=<?php echo ($vo["id"]); ?>" class="btn btn-default" onclick="treat(this,'是否删除栏目?')">删除</a>
      </td>
